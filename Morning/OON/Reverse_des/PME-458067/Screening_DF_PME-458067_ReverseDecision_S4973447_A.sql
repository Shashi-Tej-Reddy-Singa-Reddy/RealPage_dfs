...
File Name:  Screening_DF_PME-458067_ReverseDecision_S4973447_A

Elijah Hanson
Kaidin Dillard
4972920 AION MANAGEMENT LLC
4973447 THE MERIDIAN NORTH
Unit A2045-B
Amya Marie Hall
App Group ID: 11643
Lease Start 07/26/2025
...
WHERE ApGId in(11643)


PRINT 'List the screeningxglog'
	SELECT *
	FROM screeningxglog
	WHERE (appid in(11948,11949) or AppGID in (11643))
...
SELECT *
FROM ScreeningXgDecision  WHERE ApGId in (11643)

PRINT 'Update:Reverse decision'
Update screeningXgDecision
set dtID = 0,deccode = 0
where DecID = 12710 and apgid = 11643
...
VALUES ('11643', GETDATE(), 'SUBMIT', 'datafix - Reverse decision to no decision.', '1', '1', GETDATE(),'22228', NULL,'4380735')
